...
/**
 * Using the clusters created by StudentBuilder, assign every student marks in all of their courses and NBTs
 * @author Josh
 */
public class MarkBuilder {
	...
	public static ArrayList<String> toList(String in) {
		String withoutBrackets = in.replace("[", "").replace("]", "").replace("$", ","); // remove brackets and
																							// whitespace
		...
		try (BufferedReader br = new BufferedReader(new FileReader(csvFile))) {
			...
			while ((line = br.readLine()) != null) {
				...
				for (int i = 0; i < numStudents; i++) {
					...
					if (i == 0) {
						writeFile(rootsFile, studentID + ", " + major1 + ", " + major2 + ", " + curriculumHash + ", "
								+ curriculumSize);
					} else {
						// studentsWriter.append(studentID + ", " + major1 + ", " + major2 + ", " +
						// curriculumHash + ", " + curriculumSize);
						// studentsWriter.close();
						writeFile(studentsFile, studentID + ", " + major1 + ", " + major2 + ", " + curriculumHash + ", "
								+ curriculumSize);
					}
